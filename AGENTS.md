...
9. **Open a follow-up cleanup PR removing `"release-as": "1.0.0"`** from the package's config block. `release-as` is a one-shot override; if it stays in place, every subsequent release of the package will be forced back to v1.0.0.

`scripts/check-release-please-state.sh` validates these invariants and runs in pre-push. It catches a `0.0.0` manifest entry without a matching `release-as`, a stale `release-as` whose value equals the manifest version, and a dead `release-as` whose value is lower than the manifest version.
