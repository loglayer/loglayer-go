...
`scripts/check-release-please-state.sh` validates these invariants and runs in pre-push. It catches both directions: a `0.0.0` manifest entry without a matching `release-as`, and a non-zero entry that still has a `release-as` left over.
