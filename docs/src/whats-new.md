...
- **Core**: fluent builder API with persistent fields, per-call metadata, errors, Go context, and group-based routing. Distinct `Fields`/`Metadata`/`Data` types so the compiler catches misuse; `loglayer.F` and `loglayer.M` are short aliases for terser call sites. Every method safe from any goroutine. See [Getting Started](/getting-started).
...
- **Plugins**: six lifecycle hooks with centralized panic recovery. Built-in: [redact](/plugins/redact), [fmtlog](/plugins/fmtlog) (opt-in `fmt.Sprintf` semantics for multi-arg messages), [datadogtrace](/plugins/datadogtrace), [oteltrace](/plugins/oteltrace). Author your own per [Creating Plugins](/plugins/creating-plugins).
...
- **Source / caller info**: opt-in via `Config.AddSource` (default off). When on, every emission captures file/line/function and surfaces them under `Config.SourceFieldName` (default `"source"`). JSON tags match the slog convention. Costs one `runtime.Caller` per emission when enabled. The slog handler forwards `Record.PC` for free, so source rendering on the slog path requires no AddSource toggle on the loglayer side. See [Configuration](/configuration#addsource-sourcefieldname).
